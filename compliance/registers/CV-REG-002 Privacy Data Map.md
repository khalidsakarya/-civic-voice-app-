...
| **Precise GPS coordinates** | Not Stored — transiently processed | Device GPS (if location permission granted) | Resolve province/territory label only | Consent (location permission) | Not stored — processed in-memory on-device only; discarded immediately after province is resolved | Not transmitted or shared | Not retained — discarded in-memory; no server-side copy | User can revoke location permission at any time in device settings | Coordinates are never written to localStorage, Firestore, or any server log; only the derived province/territory label is persisted | §4 | Precise GPS coordinates are not stored. If location permission is granted, coordinates may be processed temporarily on-device or in-memory only to determine province/territory and are then discarded. The label "Not Collected" is not used here because coordinates are transiently processed, even though they are never stored. |
