...
| **CV-ISS-020** | 2026-08-16 | Canada news feed — missing Firestore composite index | The `news_alerts` collection query uses `where('country','==','CA'), orderBy('timestamp','desc'), limit(20)`. This requires a composite index on `(country ASC, timestamp DESC)` that has not been created. The Canada news feed fails silently at runtime — no news alerts are shown to Canadian users. The browser console shows: `[CA:notifs] Firestore load failed: The query requires an index.` A Firebase console link to create the index appears in the error message. | CV-SEC-001 · CV-RPT-002 | High | Open | Technical Lead | Before public launch | Composite Firestore index created for `news_alerts` collection on `(country ASC, timestamp DESC)`; Canada news feed confirmed loading in browser | | | | **2026-08-16:** Identified during App Approval Readiness hardening pass (CV-RPT-002 §7.1). To fix: open Firebase console → Firestore → Indexes → Composite → Add index (or click the auto-link from the browser error). This is not a data or rules issue — all rules are correct. Index creation only. |
