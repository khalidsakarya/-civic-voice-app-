...
| **CV-ISS-011** | 2026-08-02 | Public Data Sources page route | The URL at which the public Data Sources page (civicvoice.ca/sources or equivalent) will be published has not been confirmed. OGL-Canada 2.0 attribution and source transparency depend on this page being live before launch. | CV-POL-002 · CV-POL-004 · CV-CHK-002 | Medium | **In Progress** | Founder · Developer | TBD | Route confirmed; Data Sources page live and accessible; OGL-Canada attribution notice present; source list from CV-REG-001 reflected on page | | | | Attribution wording from CV-POL-002 §6.2 and CV-POL-004 §6 to be used on this page. **2026-08-15 update:** In-app `#sources` route implemented and verified (CV-UI-VER-002 VER-002-07). Page displays all 3 active MVP datasets with source, table, period, licence, and transformation detail. Production URL (civicvoice.ca/sources or equivalent deep link) not yet confirmed — issue remains open. |
...
| **CV-ISS-015** | 2026-08-02 | Firestore security rules review | The Firestore security rules review defined in CV-SOP-004 §7 has not been conducted. No Firestore Rules Review Record exists. Production rules have not been validated against intended read/write behaviour. | CV-SOP-004 · CV-SEC-001 | High | **Closed** | Technical Lead | 2026-08-15 | Firestore Rules Review Record created and signed off; all collections reviewed against CV-SOP-004 §7.2 checklist; no unintended public read or write access confirmed | Firestore rules v1.0 deployed and verified post-deployment. Prior production ruleset was not exported before deployment; residual risk accepted because post-deployment functional and security verification passed. | 2026-08-15 | **2026-08-15 update:** Code audit completed (CV-SEC-001). **Critical finding: no `firestore.rules` file exists in either repository.** Production rules are unversioned and unknown. Collections used by Canadian MVP UI identified: `subnational_economic_social_stats`, `subnational_tax_exempt_entities`, `subnational_grants`, `subnational_jurisdictions`, `subnational_bills`, plus 14 other public-read collections. Client-write collections: `citizen_votes`, `vote_counts`, `surveys`, `news_votes`, `news_vote_counts`, `department_votes`, `user_events`, `fcm_tokens`. Admin SDK confirmed for engine writes. No service account keys tracked in git. `REACT_APP_FIREBASE_API_KEY` committed in `.env` (web API key — low risk, but `.env` should be gitignored). Recommended rules drafted in CV-SEC-001 §8. **2026-08-15 update (v0.9):** `firestore.rules` v1.0 draft created. `firebase.json` created. `.env` added to `.gitignore`. **2026-08-15 update (v1.0):** Rules deployed (`firebase deploy --only firestore:rules`, exit code 0). Post-deployment verification completed via browser automation — 18/18 Firestore rule checks passed, 12/12 Canadian MVP UI checks passed (CV-SEC-001 §10). **Residual:** test doc `vote_counts/rules-test-delete-me` to be deleted via Firebase console. This issue is **Ready for Review** — Founder sign-off required to close. |
...
| **CV-ISS-017** | 2026-08-02 | Exact Statistics Canada table selection for CV-DATA-001 and CV-DATA-002 | CV-DATA-001 (Statistics Canada — Population Estimates) and CV-DATA-002 (Statistics Canada — Labour Force / Unemployment) required table number confirmation before any Firestore write. | CV-PLAN-003 · CV-DATA-001 · CV-DATA-002 · CV-REC-001 · CV-SRC-REV-001 | Critical | **Partially Deferred — In Progress** | Data Lead | TBD | Exact Statistics Canada table numbers confirmed for CV-DATA-001 and CV-DATA-002; dataset URLs recorded; column and geography selections recorded; CV-PLAN-003 dataset definitions updated; CV-REC-001 completed for each table before first Firestore write | | | | **2026-08-16 update:** CV-DATA-002 (unemployment) — Table 14-10-0287-01 confirmed, written to `subnational_economic_social_stats/CA-ON`, verified (CV-UI-VER-001 VER-001-01). This half of the issue is resolved. CV-DATA-001 (population) — deferred from MVP launch scope; display slot and product decision pending. This issue remains open for CV-DATA-001 only. Not a launch blocker while CV-DATA-001 is deferred. |
| **CV-ISS-018** | 2026-08-02 | First launch dataset fetch and verification | The five selected launch datasets (CV-DATA-001, 002, 008, 013, 014) have not yet been fetched, reviewed against CV-REC-001, or verified for Firestore write. No data can appear in the app until this workflow is complete for each dataset. | CV-PLAN-003 · CV-REC-001 · CV-SOP-001 · CV-LAUNCH-DEC-001 · CV-UI-VER-001 | Critical | **Partially Complete — In Progress** | Data Lead | TBD | CV-REC-001 completed and signed off for each of the 5 selected datasets (CV-DATA-001, 002, 008, 013, 014); source URL, fetched date, reporting period, and licence status confirmed in CV-REG-001 for each; Firestore write confirmed against CV-SOP-001 | | | | **2026-08-03/04 update:** CV-DATA-002 (unemployment) — fetched, CV-REC-001 completed, written to `subnational_economic_social_stats/CA-ON`. CV-DATA-008 (CRA Charities) — fetched, CV-REC-001 completed, written to `subnational_tax_exempt_entities/CA-ON`; designation labels A/B corrected and refreshed 2026-08-04. **2026-08-05 update:** CV-DATA-002 and CV-DATA-008 visually verified in app UI (CV-UI-VER-001 VER-001-01, VER-001-02, VER-001-03). **2026-08-16 update (scope finalised):** CV-DATA-014 (Ontario Transfer Payments) written to `subnational_grants/CA-ON` and visually verified (CV-UI-VER-001 VER-001-04). **All 3 active MVP datasets are written and visually verified: CV-DATA-002 (unemployment), CV-DATA-008 (CRA charities), CV-DATA-014 (Ontario transfer payments).** CV-DATA-001 (population) and CV-DATA-013 (Ontario Budget) have been **formally deferred** from the MVP launch scope — they are not launch blockers. This issue is not closed because CV-DATA-001 and CV-DATA-013 remain unwritten; it should be re-evaluated when those product decisions are made post-launch. |
| **CV-ISS-019** | 2026-08-02 | Firestore mapping and app display location for selected launch datasets | The Firestore collection paths, document structure, and in-app display location (screen name, section, card type) for the 5 selected launch datasets (CV-DATA-001, 002, 008, 013, 014) have not been defined. CV-PLAN-003 shows TBD for all Firestore and display fields. | CV-PLAN-003 · CV-LAUNCH-DEC-001 · CV-UI-VER-001 | High | **Partially Complete — In Progress** | Founder / Developer · Data Lead | TBD | Firestore collection path and document structure defined for each of the 5 selected datasets; in-app display location (screen, section, card) confirmed; CV-PLAN-003 updated with confirmed values | | | | **2026-08-05 update:** CV-DATA-002 — `subnational_economic_social_stats/CA-ON`, displayed in Ontario Economic & Social modal; unemployment rate and series confirmed visible (CV-UI-VER-001 VER-001-01). CV-DATA-008 — `subnational_tax_exempt_entities/CA-ON`, displayed in Ontario Tax Exempt / Charities modal; 100 records confirmed visible; $0 value display fixed — "Financial value not displayed in MVP" shown (CV-UI-VER-001 VER-001-02, VER-001-03). **2026-08-16 update (scope finalised):** CV-DATA-014 Firestore path confirmed: `subnational_grants/CA-ON`, displayed in Ontario Transfer Payments modal. **Firestore paths and app display locations confirmed for all 3 active MVP datasets: CV-DATA-002, CV-DATA-008, and CV-DATA-014.** CV-DATA-001 and CV-DATA-013 have been formally deferred from MVP launch scope — Firestore paths and display locations for those two datasets will be confirmed when product decisions are made post-launch. This issue is not closed; it remains open for the deferred datasets. |
| **CV-ISS-020** | 2026-08-16 | Canada news feed — missing Firestore composite index | The `news_alerts` collection query uses `where('country','==','CA'), orderBy('timestamp','desc'), limit(20)`. This requires a composite index on `(country ASC, timestamp DESC)` that has not been created. The Canada news feed fails silently at runtime — no news alerts are shown to Canadian users. The browser console shows: `[CA:notifs] Firestore load failed: The query requires an index.` A Firebase console link to create the index appears in the error message. | CV-SEC-001 · CV-RPT-002 | High | **Closed** | Technical Lead | Before public launch | Composite Firestore index created for `news_alerts` collection on `(country ASC, timestamp DESC)`; Canada news feed confirmed loading in browser | Index deployed (`firebase deploy --only firestore:indexes`) and confirmed `READY` via Firestore Admin API. The exact failing query (`country == CA`, `orderBy timestamp desc`, `limit 20`) was replicated directly against Firestore and returned HTTP 200 with no `FAILED_PRECONDITION` / "requires an index" error. Browser console on a full app load showed no `[CA:notifs] Firestore load failed` warning and no other console errors. | 2026-08-23 | Technical Lead | **2026-08-16:** Identified during App Approval Readiness hardening pass (CV-RPT-002 §7.1). To fix: open Firebase console → Firestore → Indexes → Composite → Add index (or click the auto-link from the browser error). This is not a data or rules issue — all rules are correct. Index creation only. **2026-08-23 update:** `firestore.indexes.json` created and deployed; index reached `READY` state (~2 min build). Post-deployment verification passed — see closure note. **Observation (not a defect, not reopening this issue):** existing `news_alerts` documents use a `date` field rather than `timestamp`, so the bell-notification query (which orders by `timestamp`) currently matches zero documents and correctly renders the app's "No notifications yet" empty state rather than an error. The separate "News & Announcements" screen (`fetchNewsForCountry`, single-field `where` only, no composite index required) already shows real live Canada content. If CA bell notifications should show data going forward, `news_alerts` writes should include a `timestamp` field — tracked informally here for awareness, not opened as a new blocking issue. |
...
| 0.6 | 2026-08-16 | Founder / Data Lead | MVP dataset scope finalised. CV-DATA-001 and CV-DATA-013 formally deferred from MVP launch scope — not launch blockers. CV-ISS-017 updated to Partially Deferred — CV-DATA-002 resolved, CV-DATA-001 deferred. CV-ISS-018 updated: all 3 active MVP datasets written and verified; remaining 2 deferred (not failed). CV-ISS-019 updated: Firestore paths confirmed for all 3 active datasets; deferred datasets to be addressed post-launch. |
| 0.7 | 2026-08-15 | Founder / Compliance Lead | Public legal pages implemented and verified (commit eca504f, CV-UI-VER-002 — 8/8 Pass). All six routes (`#privacy`, `#terms`, `#accessibility`, `#sources`, `#disclaimer`, `#contact`) confirmed loading with full content. No raw TBD visible; safe wording used throughout for unresolved operator details (CV-ISS-001–004). CV-ISS-011 updated to In Progress — `#sources` route implemented; production URL not yet confirmed. CV-ISS-013 (legal review) and CV-ISS-012 (App Store privacy labels) remain open — not closed by this change. |
| 0.8 | 2026-08-15 | Founder / Technical Lead | Firestore rules code audit completed (CV-SEC-001). CV-ISS-015 updated to In Progress. Critical finding: no `firestore.rules` file in either repository — production rules are unversioned and unknown. Two launch blockers identified: F-001 (no rules file) and F-003 (unverified write protection). `REACT_APP_FIREBASE_API_KEY` committed in `.env` (medium finding — web key, low intrinsic risk, but `.env` should be gitignored). Recommended rules drafted in CV-SEC-001 §8. CV-ISS-015 remains open pending console export, rules file creation, and deployment. |
| 0.9 | 2026-08-15 | Founder / Technical Lead | `firestore.rules` v1.0 draft created in app repo. `firebase.json` created with firestore rules config. `.env` added to `.gitignore`. F-001 (no rules file) and F-002 (.env committed) resolved at repo level. CV-ISS-015 remains In Progress — rules not yet deployed to Firebase console. Remaining blockers: (1) compare drafted rules against current production console rules, (2) deploy `firebase deploy --only firestore:rules`, (3) verify in Rules Playground. |
| 1.0 | 2026-08-15 | Founder / Technical Lead | `firestore.rules` v1.0 deployed to production (`civic-voice-5ea94`), compiled successfully, exit code 0. CV-ISS-015 remains In Progress — production rules were not exported before deployment; Rules Playground verification required to confirm no regressions and that all intended read/write behaviours are correct. |
| 1.1 | 2026-08-15 | Founder / Technical Lead | Post-deployment verification completed (CV-SEC-001 §10). 18/18 Firestore rule checks passed; 12/12 Canadian MVP UI checks passed. CV-ISS-015 updated to Ready for Review. Residual: test doc `vote_counts/rules-test-delete-me` to be cleaned up via Firebase console. |
| 1.2 | 2026-08-15 | Founder / Technical Lead | CV-ISS-015 closed. Test doc `vote_counts/rules-test-delete-me` deleted via Admin SDK — confirmed `post-delete exists: false`. Closure note recorded per CV-SEC-001 v1.4. |
| 1.3 | 2026-08-16 | Founder / Technical Lead | App Approval Readiness hardening pass completed (CV-RPT-002 v1.0). CV-ISS-020 opened: Canada news feed (`news_alerts`) fails silently due to missing composite Firestore index `(country ASC, timestamp DESC)` — High priority, must fix before public launch. All other hardening checks passed. Overall recommendation: Ready with Blockers (CV-ISS-001–004, CV-ISS-012, CV-ISS-013, CV-ISS-020). |
| 1.4 | 2026-08-23 | Founder / Technical Lead | CV-ISS-020 closed. `firestore.indexes.json` created, deployed (`firebase deploy --only firestore:indexes`), confirmed `READY`. Failing query replicated directly against Firestore — returned HTTP 200, no index error. Browser console verified clean (no `[CA:notifs] Firestore load failed` warning, no other errors). Non-blocking observation logged in the issue row: existing `news_alerts` docs use a `date` field rather than `timestamp`, so the bell-notification query currently returns zero matches and correctly renders the app's empty state; the separate live "News & Announcements" screen is unaffected and already shows real content. |
