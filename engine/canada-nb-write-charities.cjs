/**
 * CA-NB (New Brunswick) — CV-DATA-NB-002 CRA Charities write ONLY.
 *
 * Writes the CRA Charities Registry (New Brunswick filter) dataset to Firestore
 * using merge-only. Does NOT touch CV-DATA-NB-001 (unemployment — automated
 * fetch still blocked by a transient network issue) or CV-DATA-NB-003 (Public
 * Payments — written separately by canada-nb-write-grants.cjs).
 *
 * Approval (2026-09-07): write CA-NB-002 CRA Charities.
 *   Target: subnational_tax_exempt_entities/CA-NB, merge-only.
 ...
 *
 * Verification record:
 *   CV-REC-001-2026-09-07-CV-DATA-NB-002 (NB CRA Charities)
 *
 * Usage:
 *   node engine/canada-nb-write-charities.cjs
 */
...
const VERIFICATION_ID = 'CV-REC-001-2026-09-07-CV-DATA-NB-002';

async function writeNbCharities(db) {
  const result = {
    dataset_id: 'CV-DATA-NB-002',
    dataset_name: 'CRA Charities Registry — New Brunswick org-level extract (MVP: name/type/category)',
  ...
  try {
    console.log('[canada-nb-write-charities] CV-DATA-NB-002: fetching…');
    ...
    doc.cv_data_id = 'CV-DATA-NB-002';

    console.log('[canada-nb-write-charities] CV-DATA-NB-002: writing to Firestore (merge)…');
    ...
    console.log(`[canada-nb-write-charities] CV-DATA-NB-002 written (${result.records_written} records).`);
  } catch (err) {
...
async function main() {
  console.log('[canada-nb-write-charities] ============================================');
  console.log('[canada-nb-write-charities] CA-NB — CV-DATA-NB-002 CRA Charities Firestore write');
  console.log('[canada-nb-write-charities] CV-DATA-NB-001 (unemployment) and CV-DATA-NB-003 (payments) are NOT touched by this script');
  console.log(`[canada-nb-write-charities] Started: ${WRITE_AT}`);
  console.log('[canada-nb-write-charities] ============================================');

  const db = tryGetFirestore();
  if (!db) {
    console.error('[canada-nb-write-charities] FATAL: Firebase credentials not found.');
    console.error('[canada-nb-write-charities] Credential source:', describeCredentialSource());
    process.exit(1);
  }
  console.log(`[canada-nb-write-charities] Firebase connected (${describeCredentialSource()})`);

  const result = await writeNbCharities(db);

  await db.terminate();

  if (!fs.existsSync(REPORTS_DIR)) fs.mkdirSync(REPORTS_DIR, { recursive: true });
  const ts = WRITE_AT.replace(/[:.]/g, '-').replace('T', '_').slice(0, 19);
  const reportPath = path.join(REPORTS_DIR, `canada-nb-write-charities-${ts}.json`);
  ...
  shared.datasets = (shared.datasets || []).filter((d) => d.dataset_id !== 'CV-DATA-NB-002');
  ...
  console.log('\n[canada-nb-write-charities] ============ WRITE SUMMARY ============');
  ...
  console.log('\n[canada-nb-write-charities] done.');

  if (result.status !== 'WRITTEN') process.exit(1);
}

main().catch((err) => {
  console.error('[canada-nb-write-charities] fatal:', err);
  process.exit(1);
});
