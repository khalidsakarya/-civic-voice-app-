...
const JOBS = [
  ...
  {
    id: 'election_processor_daily',
    tier: TIERS.DAILY,
    label: 'Election Processor — Active Countries (runs only during election periods)',
    handler: async () => {
      const countries = ['CA', 'US', 'UK', 'AU'];
      const hot = countries.filter(isElectionPeriod);
      if (!hot.length) {
        console.log('[scheduler] No countries in election period — skipping daily processor');
        return { skipped: true, reason: 'no election period active' };
      }
      console.log(`[scheduler] Election period active for: ${hot.join(', ')} — running election processor`);
      return runElectionProcessor(hot);
    },
  },
  {
    id: 'election_weekly',
  ...
  {
    id: 'election_processor_weekly',
    tier: TIERS.WEEKLY,
    label: 'Election Processor — All Countries (weekly briefing)',
    handler: async () => {
      console.log('[scheduler] Running weekly election processor (CA, US, UK, AU)');
      return runElectionProcessor(['CA', 'US', 'UK', 'AU']);
    },
  },
