...
function sourceLink(url, label = 'View official source') {
  if (!url) return null;
  return (
    <a href={url} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-600 underline mt-2 inline-block">
      {label}
    </a>
  );
}
...
/** Renders a simple status+notes object (used for lobbying, net_worth, and fallback stock_holdings) */
function StatusBlock({ data, sourceUrl }) {
