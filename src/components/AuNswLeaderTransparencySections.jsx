...
import {
  ...
  auNswNeedsManualReview,
  auNswSourceUrl,
  auNswTransparencySectionLoaded,
  formatMoney,
} from '../utils/auNswLeaderTransparency';
  ...
  return (
    <div className="mt-2">
      ...
      <p className="text-[10px] text-gray-500 mt-1.5 leading-relaxed">
        ...
        when shown on the official NSW Register of Disclosures.
      </p>
    </div>
  );
}
...
function TransparencyAccordion({ id, title, subtitle, open, onToggle, children, disabled }) {
  const panelId = `au-nsw-transparency-${id}`;
  return (
...
function DetailSalary({ row }) {
  ...
  return (
    <div className="pt-3 space-y-2">
      ...
      {sourceLink(src, 'View NSW Parliament remuneration PDF')}
    </div>
  );
}
...
function parseFinancialDisclosureSummary(fd, row) {
  ...
  const holdingCount =
    ...
    0;
  return {
    status: fd.status === 'filed' ? 'Filed' : String(fd.status || 'See NSW Parliament'),
    latestYear: fd.latest_filing_year != null ? String(fd.latest_filing_year) : '—',
    form: 'Register of Disclosures (NSW Legislative Assembly)',
    publicValues: 'Official register entries (ranges or narrative where stated)',
    leaderSpecific:
      holdingCount > 0
        ? `${holdingCount} reported interest rows from official register PDF/HTML`
        : auNswNeedsManualReview(fd)
          ? 'Open official register volumes on parliament.nsw.gov.au'
          : 'See tabled register papers for member schedules',
  };
}

function DetailFinancialDisclosure({ row }) {
  const fd = row?.financial_disclosure;
  const src = auNswSourceUrl(fd) || row?.field_sources?.financial_disclosure;
    ...
    <div className="pt-3 space-y-3">
      {summary && (
        <div className="rounded-lg border border-gray-200 bg-gray-50/90 px-3 py-1">
          ...
          <ResultRow label="Premier-specific disclosures" value={summary.leaderSpecific} />
        </div>
      )}
      ...
      <details className="rounded-lg border border-gray-200 bg-white">
        ...
        <div className="px-3 pb-3 pt-1 space-y-2 border-t border-gray-100">
          ...
          {auNswNeedsManualReview(fd) && frameworkNote()}
        </div>
      </details>

      {sourceLink(src, 'NSW Parliament Register of Disclosures')}
    </div>
  );
}
    ...
    <div className="pt-3 space-y-2">
      ...
      {rows.length > 0 ? (
        <ReportedHoldingsTable
          rows={rows}
          caption={`${rows.length} interest row(s) from NSW register (not stock purchases)`}
        />
      ) : (
        <p className="text-xs text-gray-600 italic">
      ...
      {auNswNeedsManualReview(sh) && frameworkNote()}
      {sourceLink(src, 'Register of Disclosures')}
    </div>
  );
}
...
function DetailGiftsHospitality({ row }) {
  ...
  return (
    <div className="pt-3 space-y-3">
            ...
            {gh.gifts.map((g, i) => (
              <li key={i} className="text-xs border border-gray-200 rounded-lg p-2.5 bg-gray-50">
                ...
                <p className="text-gray-500 mt-0.5">
                  {g.date ? `${g.date} · ` : ''}
                  {g.value_text || (g.value != null ? formatMoney(g.value, 'AUD', 2) : '')}
                </p>
              </li>
            ))}
          </ul>
            ...
            {gh.travel_payments.map((t, i) => (
              <li key={i} className="text-xs border border-gray-200 rounded-lg p-2.5 bg-gray-50">
                ...
                <p className="text-gray-800 mt-0.5">
                  {t.amount_text || (t.amount != null ? formatMoney(t.amount, 'AUD', 2) : '')}
      ...
      {sourceLink(src, 'Official gifts register')}
    </div>
  );
}
            ...
            {cf.items.map((it, i) => (
              ...
              >
                ...
                <span className="font-medium shrink-0">
                  {it.amount_text || (it.amount != null ? formatMoney(it.amount, 'AUD', 2) : '')}
                </span>
              </li>
            ))}
          </ul>
    ...
    <div className="pt-3 space-y-2">
      ...
      {lr?.status === 'official_data_requires_manual_review' && (
        <p className="text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
          {lr.error
            ? `NSW Lobbying register could not be loaded: ${lr.error}`
            : 'Lobbying registrations are published on lobbying.nsw.gov.au; automated fetch was not available from this environment.'}
        </p>
      )}
      {rows.length > 0 && (
        <p className="text-[10px] text-gray-500 leading-relaxed">
          Payment amounts are not always published in the public NSW Lobbying Commissioner register.
        </p>
      )}
      {sourceLink(lr?.portal, 'NSW Lobbying Commissioner portal')}
      {sourceLink(src)}
    </div>
  );
}
...
function DetailRecentActivity({ row }) {
  ...
  return (
    <div className="pt-3 space-y-2">
      ...
      {sourceLink(src, 'NSW Government media releases')}
    </div>
  );
}
 ...
 */
export default function AuNswLeaderTransparencySections({ transparencyRow, loading }) {
  ...
  const summaryCards =
    !loading && transparencyRow ? buildAuNswTransparencySummaryCards(transparencyRow) : [];

  return (
    <section className="au-nsw-leader-transparency">
      ...
      {!loading && (
        <div>
          ...
          <div className="space-y-2">
            {AU_NSW_TRANSPARENCY_SECTIONS.map(({ key, label }) => {
              const loaded = auNswTransparencySectionLoaded(key, transparencyRow);
              const Detail = DETAIL_COMPONENTS[key];
              const subtitle = loaded ? auNswAccordionSubtitle(key, transparencyRow) : '';
